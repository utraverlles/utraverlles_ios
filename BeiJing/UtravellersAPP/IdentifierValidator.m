...
//    return [pinYin substringToIndex:1];
    ...
    char *cvalue = [pinYin UTF8String];
    int len = strlen(cvalue);
    for (int i = 0; i < len; i++)
    {
        if (!(cvalue[i] >= '0' && cvalue[i] <= '9') && !(cvalue[i] >= 'A' && cvalue[i] <= 'Z'))
        {
            cvalue[i] = '~';
        }
    }
    pinYin = [NSString stringWithCString:cvalue encoding:NSUTF8StringEncoding];
...
+ (void)showCancelAlert:(UIViewController*)viewController msg:(NSString *)msg
{
    ...
    NSString *cancelButtonTitle = NSLocalizedString(@"关闭", nil);
    UIAlertController *alertController = [UIAlertController alertControllerWithTitle:title message:message preferredStyle:UIAlertControllerStyleAlert];
    // Create the actions.
    UIAlertAction *cancelAction = [UIAlertAction actionWithTitle:cancelButtonTitle style:UIAlertActionStyleCancel handler: ^(UIAlertAction *action) {
        //NSLog(@"The \"Okay/Cancel\" alert's cancel action occured.");
    }];
    [cancelAction setValue:[UIColor colorWithRed:255.0 / 255 green:185.0 / 255 blue:0 / 255 alpha:1.0] forKey:@"_titleTextColor"];
